    ...
    $(document).ready(function(){
		...
		$('.purchase-options input').change(function(){
			...
			if (subscriptionType == 'one-time-purchase'){ 
				...
				updatePurchaseOptions($('.purchase-options select').val());
		...
		*/
		...
		function updatePurchaseOptions(v){
			...
			localStorage.setItem('subpack', v);
...
foreach ($options as $option) {
	...
	if ($option['value'] == 0) {
		...
		$one_time_option = "<li". ($option['selected'] ? " class='selected'" : "") .">".
			...
			"<label for='one-time-purchase' class='one-time-purchasecs'>". __('One-off Purchase') ."</label></li>\n";
		...
		$subscription_options[] = "<option value='". $option['value'] ."'". ($option['selected'] ? " selected" : "") .">"."Every ".
...
<div class='single-pro-acc seco-box-ac'>
<a class='accod-btn one-off collapsed' data-toggle='collapse' href='#collapse28' role='button' aria-expanded='false' aria-controls='collapseExample'>
	<div class='acc-head'>
		<p>YOU ARE MAKING A ONE-OFF ORDER</p>
	</div>
	<div class='acc-icon'>
...
<a class='accod-btn subs collapsed' data-toggle='collapse' href='#collapse28' role='button' aria-expanded='false' aria-controls='collapseExample' style='display:none;'>
	<div class='acc-head'>
		<p>YOU ARE SETTING UP A SUBSCRIPTION</p>
	</div>
	<div class='acc-icon'>
		<i class='fa fa-minus open-down-up' aria-hidden='true'></i>
...
<div class='collapse space-sco' id='collapse28'>
...
".
	...
	($subscription_options ? "<li". (isset($selected_subscription_option) ? " class='selected'" : "") .">".
		...
		"<label for='subscriptions-list' class='subscriptions-listcs'>". __('Subscribe & Save') ."</label>\n".
		"<select name='subscription-options' id='subscription-options'>". implode('', $subscription_options) ."</select>\n" : "").
