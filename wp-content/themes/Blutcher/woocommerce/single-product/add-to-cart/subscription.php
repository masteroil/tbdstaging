...
<div class="btn_prod">
    <?php 
		if( has_term( array( 'combo-boxes', 'raw-meals' ), 'product_cat', $product->get_id() ) ) {
			?>
    <?php $cookie_pin = isset($_COOKIE['valid_pincode'])?$_COOKIE['valid_pincode']:''; ?>
    <button product_id="<?php echo esc_attr( $product->get_id() ); ?>" class="show-pincode-pop-up button alt"
        style="<?php echo empty($cookie_pin) ? "" : "display: none;"; ?>"><?php echo esc_html( $product->single_add_to_cart_text() ); ?></button>

    <button id="add_to_cart_btn_<?php echo esc_attr( $product->get_id() ); ?>" onclick="this.innerHTML = 'Add more'" type="submit"
        class="on-pincode-required single_add_to_cart_button button alt" name="add-to-cart"
    ...
    <?php
			...
			?>
    <button id="add_to_cart_btn_<?php echo esc_attr( $product->get_id() ); ?>" type="submit"
        class="single_add_to_cart_button button alt" name="add-to-cart"
