...
class UpdatesManager extends SingletonAbstract {
	public $menu_slug; // parent menu slug to attach "License" submenu to
	public $prefix; // prefix for internal settings
	public $url; // plugin host site URL for EDD SL
	public $version; // plugin version for EDD SL
	public $name; // plugin name for EDD SL
	public $key_statuses; // store list of key statuses and messages
	public $bad_key_statuses; // store list of key statuses and messages
	public $activate_errors; // store list of activation errors and error messages
	public $last_activation_error; // because we can't pass variables directly to admin_notice
	public $theme    = false; // do we have a theme or a plugin?
	public $beta     = false; // is this a beta?
	...
	public function save_settings() {
		...
		if ( isset( $_REQUEST[ "{$this->prefix}_mb_setting" ] ) ) {
			$settings = wc_clean( $_REQUEST[ "{$this->prefix}_mb_setting" ] ?? array() );
	...
	public function admin_page() {
		?>
		<div class="wrap">
			<form method="post" action="<?php echo esc_attr( wc_clean( $_SERVER['REQUEST_URI'] ?? '' ) ); ?>">
				<?php $this->the_nonce(); ?>
	...
	public function admin_page_fields() {
		...
		<div class="cfw-admin-field-container">
			<label for="<?php echo esc_attr( $this->get_field_name( 'license_key' ) ); ?>" class="block text-sm font-medium text-gray-700">
		...
		<?php if ( ! empty( $key_status ) ) : ?>
			<div id="cfw-activation-control" class="cfw-admin-field-container">
				...
				<?php elseif ( 'inactive' === $key_status || 'site_inactive' === $key_status ) : ?>
					...
					<p class="mt-2" style="color:red;"><?php echo esc_html( $this->key_statuses[ $key_status ] ); ?></p>
				<?php elseif ( 'valid' === $key_status ) : ?>
					<input type="submit" name="deactivate_key" class="button-secondary" value="Deactivate Site" />
					<p class="mt-2" style="color:green;"><?php echo esc_html( $this->key_statuses[ $key_status ] ); ?></p>
				<?php else : ?>
					<p class="mt-2" style="color:red;"><?php echo esc_html( $this->key_statuses[ $key_status ] ); ?></p>
				<?php endif; ?>
			</div>
	...
	public function get_license_data() {
		...
		if ( is_wp_error( $response ) ) {
			wc_get_logger()->error( 'CheckoutWC License Check Error: ' . $response->get_error_message(), array( 'source' => 'checkout-wc' ) );
			wc_get_logger()->error( wc_print_r( $response, true ), array( 'source' => 'checkout-wc' ) );
			return false;
		}
	...
	public function ajax_save_license() {
		...
		$this->set_field_value( 'license_key', wc_clean( $_REQUEST['key'] ?? '' ) );
		$this->auto_activate_license();

		ob_start();
		$this->admin_page_activation_status_button();
		...
		wp_send_json(
			array(
				'success'   => true,
				'fragments' => array(
					'#cfw-activation-control' => $cfw_activation_control_content,
				),
			)
		);
	}
