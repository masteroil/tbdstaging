...
class AssetManager {
	...
	public function set_global_assets() {
		...
		if ( PlanManager::can_access_feature( 'enable_side_cart' ) ) {
			wp_enqueue_style( 'cfw_side_cart_css', "{$this->front}/css/checkoutwc-side-cart-{$this->version}{$this->min}.css", array(), CFW_VERSION );
			wp_enqueue_script( 'cfw_side_cart_js', "{$this->front}/js/checkoutwc-side-cart-{$this->version}{$this->min}.js", array( 'jquery' ), CFW_VERSION, true );
	...
	public function set_cfw_page_assets() {
		...
		if ( cfw_is_checkout() ) {
			...
			wp_enqueue_style( 'cfw_front_css', "{$this->front}/css/checkoutwc-order-pay-{$this->version}{$this->min}.css", array(), CFW_VERSION );

			wp_enqueue_script( 'woocommerce', "{$this->front}/js/checkoutwc-order-pay-{$this->version}{$this->min}.js", array( 'cfw_vendor_js' ), CFW_VERSION, true );
		} elseif ( is_order_received_page() ) {
			wp_enqueue_style( 'cfw_front_css', "{$this->front}/css/checkoutwc-thank-you-{$this->version}{$this->min}.css", array(), CFW_VERSION );
			...
			if ( PlanManager::can_access_feature( 'enable_map_embed' ) && PlanManager::can_access_feature( 'enable_thank_you_page' ) ) {
				wp_enqueue_script( 'cfw-google-places', "https://maps.googleapis.com/maps/api/js?key=$google_api_key", array( 'woocommerce' ), CFW_VERSION, true );
			}
			wp_enqueue_script( 'woocommerce', "{$this->front}/js/checkoutwc-thank-you-{$this->version}{$this->min}.js", array( 'cfw_vendor_js' ), CFW_VERSION, true );
		}
		...
		if ( cfw_is_checkout() || cfw_is_checkout_pay_page() ) {
			wp_enqueue_script( 'selectWoo', "{$this->front}/js/checkoutwc-selectwoo-{$this->version}{$this->min}.js", array( 'jquery' ), '1.0.6', true );
		...
		return apply_filters(
			...
			array(
				...
				'messages'        => array(
					...
					'account_already_registered_notice' => apply_filters( 'woocommerce_registration_error_email_exists', cfw__( 'An account is already registered with your email address. <a href="#" class="showlogin">Please log in.</a>', 'woocommerce' ), '' ), // phpcs:ignore WooCommerce.Commenting.CommentHooks.MissingHookComment
