...
class SmartyStreetsAddressValidationAction extends CFWAction {
	...
	public function action() {
		...
		try {
			...
			$this->out(
				array(
					'code'                   => 2,
					'address'                => stripslashes( $this->format_suggested_address( $original_address, $suggested_address ) ),
					'original'               => stripslashes( WC()->countries->get_formatted_address( $original_address ) ),
					'components'             => $suggested_address,
					'suggested_button_label' => __( 'Use Recommended', 'checkout-wc' ),
					'user_button_label'      => __( 'Use Your Address', 'checkout-wc' ),
				)
			);
		} catch ( Exception $ex ) {
			$this->out(
				array(
					...
					'suggested_button_label' => __( 'Reenter Address', 'checkout-wc' ),
					'user_button_label'      => __( 'Use Existing Address', 'checkout-wc' ),
				)
			);
		}
	}
	...
	public function getDomesticAddressSuggestion( array $address, USStreetApiClient $client ): array {
		...
		if ( null === $first_candidate->getAnalysis()->getDpvMatchCode() ) {
			throw new Exception( __( 'We are unable to verify your address.', 'checkout-wc' ), 3 );
		}
