...
class StatCollection extends SingletonAbstract {
	 ...
	 * @access private
	 */
	private $approved_woocomerce_report_stats;
	...
	public function init() {
		...
		$this->approved_woocomerce_report_stats = array(
			'total_sales',
			'total_orders',
			'total_items',
		);
	...
	public function get_woo_order_stats(): array {
		...
		// For non-HPOS
		remove_filter( 'woocommerce_order_data_store_cpt_get_orders_query',
			array(
				$this,
				'handle_custom_query_var'
			)
	...
	public function admin_notice() {
		...
		<div class="notice notice-info" style="display: block !important;">
			<h3 style="font-weight: 500">
				<?php cfw_e( 'Help us improve CheckoutWC.', 'checkout-wc' ); ?>
			</h3>
			<p>
				<?php cfw_e( 'Gathering usage data helps us to improve CheckoutWC. Opt-out at anytime.' ); ?>
				<?php echo ' <a target="_blank" href="https://www.checkoutwc.com/checkout-for-woocommerce-usage-tracking/">' . cfw_esc_html__( 'Read more about what we collect.', 'woocommerce' ) . '</a>'; ?>
			</p>
			<p>
				<a href="<?php echo esc_url( $optin_url ); ?>" class="button-secondary">
					<?php cfw_e( 'Allow', 'checkout-wc' ); ?>
				</a>
				&nbsp;
				<a href="<?php echo esc_url( $optout_url ); ?>" class="button-secondary">
					<?php cfw_e( 'Do not allow', 'checkout-wc' ); ?>
				</a>
			</p>
		</div>
		<?php
		...
		if ( ! array_key_exists( $range, self::CFW_STORE_AGE_RANGES ) ) {
			throw new \InvalidArgumentException(
				sprintf(
					...
					$range,
					implode( ', ', array_keys( self::CFW_STORE_AGE_RANGES ) )
				)
			);
		}
