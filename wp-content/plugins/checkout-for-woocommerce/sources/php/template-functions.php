    ...
    <div class="cfw-display-table cfw-w100">
        <a id="cfw-expand-cart" class="cfw-display-table-row">
            ...
            <span class="cfw-cart-summary-label-show cfw-small cfw-display-table-cell">
                ...
                <svg width="11" height="6" xmlns="http://www.w3.org/2000/svg" class="cfw-arrow" fill="#000">
                    <path
...
function cfw_wc_print_notices() {
    ...
    <div id="cfw-alert-container" class="woocommerce-notices-wrapper">
        ...
        <div
            class="cfw-alert <?php echo esc_attr( $type_class_mapping[ $type ] ); ?> cfw-alert-<?php echo esc_attr( $alert_id ); ?>">
            <div class="message">
...
function cfw_order_review_tab_heading() {
    ...
    <h3>
		 ...
		 *
		 * @param $order_review_tab_heading string Order review tab heading
		 * @since 2.0.0
		 */
...
function cfw_maybe_show_already_have_an_account_text() {
	...
	?>
    <div
        class="cfw-have-acc-text cfw-small <?php echo WC()->checkout()->is_registration_required() ? 'account-does-not-exist-text' : ''; ?>">
        <?php
		/**
    ...
    <div id="cfw-account-password-slide" class="cfw-input-wrap-row">
		...
		if ( SettingsManager::instance()->get_setting( 'registration_style' ) === 'woocommerce' ) :
			if ( 'no' === get_option( 'woocommerce_registration_generate_username' ) ) {
				woocommerce_form_field( 'account_username', array(
					'type'         => 'text',
...
function cfw_maybe_show_welcome_back_text() {
    ...
    <div class="cfw-have-acc-text cfw-small">
		...
		if ( apply_filters( 'cfw_show_logout_link', false ) ) :
			?>
        <a
            href="<?php echo esc_attr( wp_logout_url( wc_get_checkout_url() ) ); ?>"><?php esc_html_e( 'Log out.', 'checkout-wc' ); ?></a>
        <?php endif; ?>
    </div>
    <?php
}
    ...
    <div id="cfw_login_modal" style="display: none;">
        <form id="cfw_login_modal_form" class="checkoutwc" method="post">
            ...
            <div class="cfw-login-modal-navigation">
                ...
                <?php if ( ! WC()->checkout()->is_registration_required() ) : ?>
                <a id="cfw_login_modal_close"
                    href="javascript:"><?php esc_html_e( 'Or continue as guest', 'checkout-wc' ); ?></a>
                <?php endif; ?>
            </div>
...
function cfw_customer_info_address() {
    ...
    <div id="cfw-customer-info-address"
        class="cfw-module <?php echo ! WC()->cart->needs_shipping() || wc_ship_to_billing_address_only() ? 'billing' : 'shipping'; ?>">
        <?php
        ...
        <div
            class="cfw-customer-info-address-container cfw-parsley-shipping-details <?php cfw_address_class_wrap( WC()->cart->needs_shipping() ); ?>">
    ...
    <ul class="cfw-review-pane cfw-module">
        <li>
            ...
            <div role="cell" class="col-2 cfw-review-pane-link">
                ...
                <a href="javascript:" data-tab="#cfw-customer-info"
                    class="cfw-tab-link cfw-small"><?php esc_html_e( 'Change', 'checkout-wc' ); ?></a>
                <?php endif; ?>
            </div>
        </li>
    ...
    <ul class="cfw-review-pane cfw-module">
        ...
        <li class="shipping-method">
            ...
            <div role="cell" class="col-2 cfw-review-pane-link">
                ...
                <a href="javascript:" data-tab="#cfw-shipping-method"
                    class="cfw-tab-link cfw-small"><?php esc_html_e( 'Change', 'checkout-wc' ); ?></a>
                <?php endif; ?>
            </div>
        </li>
...
function cfw_order_review_step_review_pane() {
    ...
    <ul class="cfw-review-pane cfw-module">
        ...
        <li class="cfw-review-pane-payment-row">
            ...
            <div role="cell" class="col-2 cfw-review-pane-link">
                <a href="javascript:" data-tab="#cfw-payment-method"
                    class="cfw-tab-link cfw-small"><?php esc_html_e( 'Change', 'checkout-wc' ); ?></a>
            </div>
        </li>
    </ul>
    ...
    <div class="cfw-force-hidden">
        <div id="ship-to-different-address">
            <input id="ship-to-different-address-checkbox" type="checkbox" name="ship_to_different_address"
                value="<?php echo WC()->cart->needs_shipping_address() ? 1 : 0; ?>" checked="checked" />
        </div>
    </div>
    <?php
        ...
        <div class="cfw-promo-wrap <?php echo $hide_promo_code_by_default ? 'cfw-hidden' : ''; ?>">
            <div class="row cfw-promo-row cfw-input-wrap-row">
                ...
                <div class="cfw-input-wrap cfw-button-input col-lg-4 cfw-promo-code-button-wrap form-row">
                    <input type="button" name="cfw-promo-code-btn" id="<?php echo esc_attr( $button_id ); ?>"
                        class="cfw-secondary-btn" value="<?php echo esc_attr( $promo_code_button_label ); ?>" />
                </div>
            </div>
        </div>
        <?php endif; ?>
...
function cfw_thank_you_title( WC_Order $order ) {
    ...
    <div class="cfw-mb">
        ...
        <a href="<?php echo esc_url( $order->get_checkout_payment_url() ); ?>"
            class="cfw-secondary-btn"><?php cfw_esc_html_e( 'Pay', 'woocommerce' ); ?></a>
    </div>
    <?php endif; ?>
    <div class="title">
        <?php
    ...
    <div class="inner status-row">
        ...
        <ul class="status-steps">
            ...
            <?php
				foreach ( $order_statuses as $order_status ) :
					...
					?>
            <li
                class="status-step <?php echo $order->get_status() === $order_status->get_slug() ? 'status-step-selected' : ''; ?>">
        ...
        <ul class="status-steps">
            ...
            <?php
					...
					?>
            <li
                class="status-step <?php echo $order->get_status() === $order_status ? 'status-step-selected status-step-current' : ''; ?>">
...
function cfw_thank_you_order_updates( WC_Order $order ) {
	?>
    <h3><?php echo apply_filters( 'cfw_order_updates_heading', __( 'Order updates', 'checkout-wc' ), $order ); ?></h3>
    <?php
	/**
    ...
    <form <?php cfw_form_attributes( 'login_form', false, false ); ?>>
        ...
        <p class="form-row">
            ...
            <button type="submit" class="woocommerce-button button woocommerce-form-login__submit" name="login"
                value="<?php cfw_esc_attr_e( 'Login', 'woocommerce' ); ?>"><?php esc_html_e( 'Login', 'woocommerce' ); ?></button>

            <span class="login-optional cfw-small">
                <a
                    href="<?php echo esc_url( wp_lostpassword_url() ); ?>"><?php cfw_esc_html_e( 'Lost your password?', 'woocommerce' ); ?></a>
            </span>
        </p>
...
function cfw_output_checkout_tabs() {
    ...
    <div id="<?php echo esc_attr( $tab_id ); ?>"
        class="cfw-panel <?php echo esc_attr( join( ' ', $tab['classes'] ) ); ?>">
        <?php
...
function cfw_output_empty_cart_message() {
	echo '<h2 id="cfw_empty_side_cart_message">' . apply_filters( 'cfw_empty_side_cart_heading', __( 'Your Cart is Empty', 'checkout-wc' ) ) . '</h2>';
}