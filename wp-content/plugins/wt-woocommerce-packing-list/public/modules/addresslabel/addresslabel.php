...
class Wf_Woocommerce_Packing_List_Addresslabel
{
	...
	*/
	...
	public function alter_customize_inputs($fields, $type, $template_type)
	{
		if($template_type==$this->module_base)
		{
			...
			elseif($type == "addresslabel_address"){
				$fields = array(
					array(
						'label'=>__('Address Type','wf-woocommerce-packing-list'),
						'type'=>'select',
						'event_class'=>'wf_cst_change_addrtype',
		...
		{			
			...
			{	
				$new_html=$match_arr[0].'<div class="wfte_addr_col">
					<div class="wfte_addresslabel_address">
				...
				$match_arr = self::getElmByClass('wt_pklist_last_dom_id', $html);
				if($match_arr){
					$inner_html=str_replace(($match_arr[0]), '', $html);
					$html = self::set_inner_html($elm_class,$inner_html);
				}
			}
		}
		return $html;
    ...
    */
	...
	protected function generate_preview_html($html)
	{
		...
		.wfte_addresslabel_address{ display:block; text-align:left; line-height:'.$current_line_height.'px; width:95%;}
	...
	public function get_customizable_items($settings,$base_id)
	{
		if($base_id==$this->module_id)
		{
			...
			return array(
				...
				'addresslabel_address' => __('Address Type','wf-woocommerce-packing-list'),
			);
		}
		return $settings;
	}
	...
	*/    
    ...
    public function generate_order_template($orders,$page_title)
    {
    	...
    	if($html!="")
    	{
			...
			.wfte_addresslabel_address{ display:inline-block; text-align:start; width:100%;}
