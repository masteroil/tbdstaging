...
class Wf_Woocommerce_Packing_List_Invoice
{
    ...
    public $plugin_pay_later_payment = "";
	public function __construct()
	{
		...
		$this->dc=Wf_Woocommerce_Packing_List::load_modules('dc');	
	...
	*/
	public function alter_customize_info_text($info_text,$type,$template_type)
	{
		if($template_type==$this->module_base)
		{
			...
			{
				$info_text=sprintf(__('You can control the visibility of the seal according to order status via filters. See filter documentation %s here. %s', 'wf-woocommerce-packing-list'), '<a href="'.admin_url('admin.php?page=wf_woocommerce_packing_list#wf-help#filters').'" target="_blank">', '</a>');
			}
		}
		return $info_text;
	}
	...
	*/
	...
	public function run_payment_link_module($plugin_name,$plugin_version){
		 ...
		 * side of the site.
		 */
		//Wf_Woocommerce_Packing_List_Pay_Later_Payment::init($plugin_name,$plugin_version);
	...
	public function wf_allow_payment_for_order_status($statuses, $order){
		...
		if(!in_array('on-hold',$statuses) && ($order_status === "on-hold")){
			$statuses[] = 'on-hold';
		}elseif(!in_array('failed',$statuses) && ($order_status === "failed")){
			$statuses[] = 'failed';
		}
		return $statuses;
	}
	...
	public function generate_auto_invoice_number()
	{
		...
		if(!empty($generate_invoice_for)){
			...
			if(!empty($empty_invoice_order_ids)){
				foreach($empty_invoice_order_ids as $this_order){
					$order_id = (int)$this_order->ID;
	...
	*/
	public function default_settings($settings,$base_id)
	{
		...
		{
			$settings=array(
				'wf_'.$this->module_base.'_contactno_email'=>array('email','contact_number','vat','cus_note','ssn'),
	        	'wf_'.$this->module_base.'_product_meta_fields'=>array(),
	...
	*/
	public function single_checkbox_fields($settings,$base_id){
		...
		{	
			...
			$settings = array(
				...
				'wt_pklist_show_individual_tax_column'=>"No",
				'woocommerce_wf_use_latest_settings_invoice' => "No",
				'wf_woocommerce_invoice_prev_install_orders' => "No",
	...
	/*
	*	@since v4.4.0 - Changed the radio button fields to multi checkbox
	*	This function is for getting the values for checkbox fields when they are unchecked,
	*	since the PHP will sent the $_POST the unchecked fields.
	*/
	public function multi_checkbox_fields($settings, $base_id){
		if($base_id == $this->module_id){
			$settings = array(
				...
				'wt_pklist_individual_tax_column_display_option' => 'amount',
	...
	public function save_multi_checkbox_fields($result,$key,$fields,$base_id){
		if($base_id == $this->module_id){
			/* Save the total tax column field */
			if($key == 'wt_pklist_total_tax_column_display_option'){
				$result = "amount";
				if(isset($_POST['wt_pklist_total_tax_column_display_option'])){
					if((is_array($_POST['wt_pklist_total_tax_column_display_option'])) && (!empty($_POST['wt_pklist_total_tax_column_display_option']))){
						$tot_tax_dis_arr = $_POST['wt_pklist_total_tax_column_display_option'];
						if(in_array('amount',$tot_tax_dis_arr) && in_array('rate',$tot_tax_dis_arr)){
							$result = 'amount-rate';
						}else{
							$result = $tot_tax_dis_arr[0];
						}
					}
				}
				if(!in_array($result,array('amount','rate','amount-rate'))){
					$result = "amount";
				}
			}elseif($key == 'wt_pklist_individual_tax_column_display_option'){
				$result = "amount";
				if(isset($_POST['wt_pklist_individual_tax_column_display_option'])){
					if((is_array($_POST['wt_pklist_individual_tax_column_display_option'])) && (!empty($_POST['wt_pklist_individual_tax_column_display_option']))){
						$ind_tax_dis_arr = $_POST['wt_pklist_individual_tax_column_display_option'];
						if(in_array('amount',$ind_tax_dis_arr) && in_array('rate',$ind_tax_dis_arr)){
							$result = 'amount-rate';
						}elseif(in_array('separate-column',$ind_tax_dis_arr)){
							$result = 'separate-column';
						}else{
							$result = $ind_tax_dis_arr[0];
						}
					}
				}

				if(!in_array($result,array('amount','rate','amount-rate','separate-column'))){
					$result = "amount";
				}
			}else{
	...
	*/
	...
	{	
		if($this->is_show_frontend_print_button($order,"order_listing"))
		{
	...
	*/
	...
	{	
		$button_page = "order_details";
		if($email_button){
			$button_page = "order_email";
		}

		if($this->is_show_frontend_print_button($order,$button_page))
		{
			$wt_actions[$this->module_base]=array(
				'print'=>array(
	...
	*/
	public function is_show_frontend_print_button($order,$button_page)
	{	
		$free_order_enable = Wf_Woocommerce_Packing_List::get_option('wf_woocommerce_invoice_free_orders',$this->module_id);
		...
		$template_type=$this->module_base;
		$show_print_button_pages = apply_filters('wt_pklist_show_hide_print_button_in_pages',true,$button_page,$template_type,$order);
		...
		if($show_print_button_pages){
			...
			if(('Yes' === $show_on_frontend) && (in_array($button_page,$show_print_button_arr)))
			{
				$generate_invoice_for=Wf_Woocommerce_Packing_List::get_option('woocommerce_wf_generate_for_orderstatus',$this->module_id);
    ...
    */
    public function generate_order_template_for_invoice_preview($orders, $page_title, $html="")
    {
    	...
    	$footer_to_the_bottom = apply_filters('wt_pklist_footer_to_the_bottom',false,$order,$template_type);
