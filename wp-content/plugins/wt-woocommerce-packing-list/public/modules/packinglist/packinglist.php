...
class Wf_Woocommerce_Packing_List_Packinglist
{
	...
	public function __construct()
	{
		...
		*/
		...
		if($vl=='Yes')
		{
			...
			// add_action('woocommerce_checkout_order_processed',array($this,'send_separate_email_for_new_order'),10,3);
	...
	*/
	...
	function send_separate_email_for_new_order($order_id, $posted_data, $order){
		$is_send_on_checkout_process = false;
		$is_send_on_checkout_process = apply_filters('wt_pklist_send_seperate_mail_only_on_checkout_process',$is_send_on_checkout_process,$this->module_base);
		if($is_send_on_checkout_process){
			WC()->mailer()->emails['Wf_Woocommerce_Packing_List_Packinglist_Email']->trigger($order_id, $order);
		}
	}

	function send_separate_email_for_new_order_tq($order_id){
		$order = wc_get_order( $order_id );
		$already_sent = get_post_meta($order_id, 'sent_packinglist_email', true);
		if(empty($already_sent)){
			$order_status = ( WC()->version < '2.7.0' ) ? $order->status : $order->get_status();
			$not_allowed_status = array('cancelled','failed');
			$not_allowed_status = apply_filters('wt_pklist_status_not_send_seperate_attachment',$not_allowed_status,$this->module_base);
			if(!in_array($order_status,$not_allowed_status)){
				$is_send_on_checkout_process = false;
				$is_send_on_checkout_process = apply_filters('wt_pklist_send_seperate_mail_only_on_checkout_process',$is_send_on_checkout_process,$this->module_base);
				if($is_send_on_checkout_process){
					WC()->mailer()->emails['Wf_Woocommerce_Packing_List_Packinglist_Email']->trigger($order_id, $order);
					update_post_meta($order_id, 'sent_packinglist_email', true);
				}
			}
		}
	}
	...
	*/
	...
	public function hide_empty_elements($hide_on_empty_fields,$template_type)
	{
		if($template_type==$this->module_base)
		{
			...
			$hide_on_empty_fields[]='wfte_package_no';
			$hide_on_empty_fields[]='wfte_total_no_of_items';
		}
	...
	public function generate_template_html($find_replace,$html,$template_type,$order,$box_packing=null,$order_package=null)
	{
		...
		{ 
			...
			$find_replace=Wf_Woocommerce_Packing_List_CustomizerLib::set_package_weight_and_count_info($find_replace,$template_type,$order,$box_packing,$order_package);
	...
	public function get_customizable_items($settings,$base_id)
	{
		if($base_id==$this->module_id)
		{
			...
			return array(
				...
				'package_no' => __('Package Number','wf-woocommerce-packing-list'),
				'total_no_of_items' => __('No of Items','wf-woocommerce-packing-list'),
	...
	public function default_settings($settings,$base_id)
	{
		if($base_id==$this->module_id)
		{
			return array(
				...
				'wt_'.$this->module_base.'_product_attribute_fields'=>array(),
				'sort_products_by'=>'',
				'bundled_product_display_option'=>'main-sub',
			);
		}else
		{
			return $settings;
