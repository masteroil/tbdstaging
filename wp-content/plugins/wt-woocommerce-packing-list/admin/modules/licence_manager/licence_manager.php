...
class Wf_Woocommerce_Packing_List_Licence_Manager
{
	...
	public function __construct()
	{
		...
		*/
		...
		add_action( 'after_plugin_row_' .WF_PKLIST_PLUGIN_BASENAME, array( $this, 'add_license_notification' ), 10, 2 );
	}
	...
	*/
	...
	public function add_license_notification( $file, $plugin ) {
		...
		$license_data = $this->get_license_data_for_checking();
	...
	public function check_if_license_activated(){
		...
		if ( ('' === $license_data['status']) || (false === $license_data['status'])) {
			$message = sprintf( __( 'The plugin license is not activated. You will not receive compatibility and security updates if the plugin license is not activated. <a href="%s" target="_blank">Activate now</a>' ), $licence_data_url );
			$status  = false;
	...
	public function get_license_data_for_checking() {
		$license   = array(
			'status'        => false,
			'licence_key'   => '',
			'instance_id'   => '',
		...
		foreach ($this->products as $product_slug => $product)
		{
			$plugin_data = get_option($product_slug.'_licence_data',false);
			if($plugin_data !== false){
				...
				if((sanitize_text_field($plugin_data['status']) === "inactive") || (sanitize_text_field($plugin_data['status']) === "")){
					break;
				}
			}else{
				$license   = array(
	...
	public function get_last_error_message() {
		if ( ! $this->last_error_message ) {
			$error = get_transient($this->main_plugin_slug.'-license_last_error');
			if ( false !== $error ) {
	...
	public function set_error_message( $message ) {
		if ( false === $message ) {
			delete_transient( $this->main_plugin_slug.'-license_last_error' );
			return;
		}
		$message = wp_kses_post( $message );
		set_transient( $this->main_plugin_slug.'-license_last_error', $message, 12 * HOUR_IN_SECONDS );
