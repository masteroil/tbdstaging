		...
		if(!is_null($order))
        {
		    ...
		    if(!isset($find_replace['[wfte_product_table_shipping]'])) /* check already added */
			{
			    if(get_option('woocommerce_calc_shipping')==='yes')
			    {
			        ...
			        {
			            $shipping = Wf_Woocommerce_Packing_List_Admin::wf_shipping_formated_price($order,$template_type);
	...
	*/
	public static function generate_package_product_table_product_column_html($wc_version,$the_options,$order,$template_type,$_product,$item,$columns_list_arr)
	{
        ...
        foreach($columns_list_arr as $columns_key=>$columns_value)
        {
			...
			if($columns_key === "serial_no" || $columns_key === "-serial_no"){
				$column_data = $item["serial_no"];
			}
            elseif($columns_key==='image' || $columns_key==='-image')
            {
            ...
            elseif($columns_key==='product' || $columns_key==='-product')
            {
		        ...
		        /**
		    	*	@since 4.4.3 Compatible with Extra product option (theme complete)
		    	*/
	...
	*/
	public static function generate_product_table_product_row_html($columns_list_arr, $template_type, $order=null,$refund_order = null, $refund_id = null)
	{
		...
		if(!is_null($order))
        {
			...
			if(!is_null($refund_order) && $order->full_refunded == 0){
				    ...
				    if($_product)
				    {
				        ...
				        foreach($columns_list_arr as $columns_key=>$columns_value)
				        {
				            ...
				            elseif($columns_key==='tax' || $columns_key==='-tax')
				            {
				            	...
				            	{	
				            		$product_total=(float) ($wc_version< '2.7.0' ? $order->get_item_meta($order_item_id,'_line_total',true) : $order->get_line_total($order_item, $incl_tax, true));
				            ...
				            elseif($columns_key==='total_price' || $columns_key==='-total_price')
				            {
		                        $product_total=($wc_version< '2.7.0' ? $order->get_item_meta($order_item_id,'_line_total',true) : $order->get_line_total($order_item, $incl_tax, true));
