	...
	*/
	...
	public function out_settings_form($args)
	{
		...
		$img_url_placeholders=array(
			...
			'[wfte_fragile_url]'=>$images_path.'fragile.png',
			'[wfte_thiswayup_url]'=>$images_path.'thiswayup.png',
			'[wfte_keepdry_url]'=>$images_path.'keepdry.png',
		);
		...
		if($template_id==0) //template id=0 then new theme
		{
			...
			if($template_type!='' && $def_template>=0) // template type,default template is necessary while creating new theme
			{
				...
				if($wpdb->insert($table_name,$insert_data,$insert_data_type)) //success
				{
					...
					$success_msg=__("Template created.",'wf-woocommerce-packing-list');
					if($activate==1 || $is_active==1){
						if($is_active==0){ /* if already active so no need to call it again */
							$this->activate_theme($template_id, $template_type);
							$is_active=1;
						}
						$success_msg=__("Template created and activated.",'wf-woocommerce-packing-list');
					}
					$out=array(
						'status'=>1,
						'msg'=>$success_msg,
		...
		}else //update theme
		{
			...
			if($wpdb->update($table_name,$update_data,$update_where,$update_data_type,$update_where_type))
			{
				...
				if(isset($name_arr->template_name))
				{
					...
					$is_active=($activate==1 ? 1 : $name_arr->is_active);
				}
				$success_msg=__("Template updated.",'wf-woocommerce-packing-list');
				if($activate==1){
					$this->activate_theme($template_id, $template_type);
					$success_msg=__("Template updated and activated.",'wf-woocommerce-packing-list');
				}
				$out=array(
					'status'=>1,
					'msg'=>$success_msg,
