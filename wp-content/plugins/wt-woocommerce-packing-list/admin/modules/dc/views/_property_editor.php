...
if(is_array($editor_panel_data))
{
	...
	{
		?> 
		<div class="wt_pklist_dc_sidebar_property_group_block" data-property-group="<?php echo esc_attr($editor_panel_key);?>">
			<?php
			if(isset($editor_panel_value['title']) && $editor_panel_value['title']!="")
			{
			?>
				<div class="wt_pklist_dc_sidebar_property_hd"><?php echo esc_html($editor_panel_value['title']);?></div>
			<?php
			}
			?>
			<div class="wt_pklist_dc_sidebar_property_container">
				...
				if(isset($editor_panel_value['fields']) && is_array($editor_panel_value['fields']))
				{
					foreach ($editor_panel_value['fields'] as $field_key => $field_value)
					{
						...
						$property_specific_css_for_label=sanitize_html_class('wt_pklist_dc_property_editor_label_for_'.$field_key);
						$field_type_specific_css_for_label=sanitize_html_class('wt_pklist_dc_property_editor_label_for_input_'.$field_type);
						?>
						<div class="wt_pklist_dc_sidebar_property_block" data-property-slug="<?php echo esc_attr($property_slug);?>" data-property="<?php echo esc_attr($field_key);?>" style="<?php echo esc_attr($container_width.$container_float);?>">
							<label class="wt_pklist_dc_sidebar_property_label <?php echo $property_specific_css_for_label.' '.$field_type_specific_css_for_label;?>"><?php echo esc_html($field_value['label']);?></label>
							<?php
							$property_specific_css=sanitize_html_class('wt_pklist_dc_property_editor_input_for_'.$field_key);
							if($field_type!="")
							{
									...
									<div class="wt_pklist_dc_four_side_prop_container">
										...
										{	
											...
											<div style="float:left; width:25%">
												<label style="float:left; width:100%; text-align:left;"><?php _e(ucfirst($side), 'wf-woocommerce-packing-list');?></label>
												<?php
									...
									<div class="wt_pklist_dc_sidebar_property_button_group">
										...
										foreach($items as $item_field_key => $item_field_value)
										{
											...
											if($item_field_type=='button_checkbox')
											{
												...
												?>
												<span class="wt_pklist_dc_sidebar_property_button_group_btn wt_pklist_button_checkbox" data-values="<?php echo esc_attr(implode('|', $values));?>" data-main-value="<?php echo esc_attr($values[0]);?>" data-value="" style="margin-right:5px;" data-property="<?php echo esc_attr($item_field_key);?>"><?php echo wp_kses_post($icon);?></span>
												<?php
											}
								...
								elseif($field_value['field_type']=='custom_preset')
								{
									...
									<div style="float:left; width:49%;">
										<select class="wt_pklist_dc_preset_input">
											...
											{
												?>
												<option value="<?php echo esc_attr($preset_key);?>"><?php echo esc_html($preset_value);?></option>
												<?php
											}
											?>
										</select>
