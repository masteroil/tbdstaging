...
var pklist_dc_ajax=(function( $ ) {
	var pklist_dc_ajax=
	{
			...
			$.ajax({
				...
				success:function(data)
				{
					...
					if(data.status==1)
					{
						...
						pklist_dc.template_content = pklist_dc_source_code.get_source_html();
						pklist_dc.template_is_active=data.is_active;
						pklist_dc.template_loaded=1;
						if(data.is_active===1){
							pklist_dc.active_template_id=pklist_dc.template_id;
						}
						...
						pklist_dc.hide_empty_elements();
						pklist_dc.add_no_row_div(); /* if it was a blank template */
					}else
					{
		...
		{
			/* save button click */
			$('.wt_pklist_dc_save_btn, .wt_pklist_dc_save_btn_sub, .wt_pklist_dc_save_activate_btn').on('click', function(){
				...
				if(pklist_dc.template_id===0) /* new theme, then prompt for name */
				{
					...
					if($(this).hasClass('wt_pklist_dc_save_activate_btn') || pklist_dc.active_template_id===0){
						$('.wt_pklist_dc_set_as_active').prop('checked', true);
					}else{
						$('.wt_pklist_dc_set_as_active').prop('checked', false);
					}
				}else
				{
					pklist_dc.save_template($(this).hasClass('wt_pklist_dc_save_activate_btn'));
				}
			...
			$('.wt_pklist_dc_template_create_btn').on('click', function(){
				...
				{
					pklist_dc.save_template($('.wt_pklist_dc_set_as_active').is(':checked'));
					$('.wt_pklist_dc_template_name_wrn').hide();
					wf_popup.hidePopup();
				}
			});
			...
			/* cancel button click */
			$('.wt_pklist_dc_cancel_btn').on('click', function(){
				
				if(confirm(wt_pklist_dc_params.labels.cancel_prompt))
				{
					if(pklist_dc.template_id===0) /* new theme, then restore the default theme */
					{
		...
		save_template:function(activate)
		{	
			...
			ajax_data.activate=(activate ? 1 : 0);
				...
				success:function(data)
				{
					...
					if(data.status==1)
					{
						...
						}else
						{
							...
							pklist_dc.add_no_row_div(); /* check the template has no row. (If empty template show row adding option) */
						}
		...
		{
			var act_btn_elms=$('.wt_pklist_dc_activate_theme, .wt_pklist_dc_delete_theme, .wt_pklist_dc_save_activate_btn');
			...
			if(this.template_is_active===0)
			{
				if(this.template_id>0)
				{
					act_btn_elms.show();
				}
				$('.wt_pklist_dc_save_activate_btn').show();
			}
		},
					...
					{						
						if(typeof data.dc_slug!="undefined")
						{
							...
							var side_bar_asset_html='<div class="wt_pklist_dc_asset_item wfte_draggable wt_pklist_dc_asset_element_group_sub" title="'+wt_pklist_dc_params.labels.drag_item_tooltip+'" draggable="true" data-slug="'+wfte_name+'">'
													...
													+data.val+'</div>';
							var sidebar_asset_elm=$(side_bar_asset_html).insertAfter('.wt_pklist_dc_asset_element_group_main[data-slug="'+pklist_dc.wfte_name+'"]');
