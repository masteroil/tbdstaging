	...
	var pklist_dc_property_editor=
	{
				...
				*/
				...
				}else
				{
					...
					if(wt_pklist_dc_params.assets_add_new_item.hasOwnProperty(pklist_dc.wfte_name) && wt_pklist_dc_params.assets_add_new_item[pklist_dc.wfte_name].hasOwnProperty('sample_html'))
					{
						...
						{	
							if (value !== "" && (jQuery.isNumeric(value) || value.match(/^\d+$/))) {
								$('.wt_pklist_dc_add_new_sub_item_wrn').show().html(wt_pklist_dc_params.labels.enter_mandatory_non_numeric_fields);
						...
						}else
						{
							...
							{	
								if(value_data !== "" && (jQuery.isNumeric(value_data) || value_data.match(/^\d+$/))) {
									$('.wt_pklist_dc_add_new_sub_item_wrn').show().html(wt_pklist_dc_params.labels.enter_mandatory_non_numeric_fields);
				    				return false;
				    			}else if(value !== "" && (jQuery.isNumeric(value) || value.match(/^\d+$/))) {
									$('.wt_pklist_dc_add_new_sub_item_wrn').show().html(wt_pklist_dc_params.labels.enter_mandatory_non_numeric_fields);
				    				return false;
				    			}
		...
		apply_property_to_target_element:function(elm)
		{
			...
			*/
			...
			if(prop_arr[0]=='attr') /*  attribute not CSS */
			{
				...
				pklist_dc.source_apply_prop(source_target_elm, prop, elm_vl, 'attr');  /* to source code */
			...
			{			
				...
				*/
				...
				$.each(editable_item_arr, function(sub_elm_wfte_name, property_arr){
					...
					if(wfte_name !== "invoice_data" && sub_elm_wfte_name!=wfte_name && $.inArray(sub_elm_wfte_name, available_items)==-1)
					{
		...
		*/
		...
		set_page_property_editor:function()
		{
			...
			var wfte_name=this.page_wfte_name;
		...
		prepare_property_editor_block_html:function(elm, full_title, item_title, is_title_element)
		{
			var html='<div class="wt_pklist_dc_sidebar_tabaccord">'
					+'<div class="wt_pklist_dc_sidebar_tabaccord_hd noselect" title="'+full_title+'">'
						...
						+(elm.hasClass('wt_pklist_dc_empty_item') ? '<span class="wt_pklist_dc_sidebar_tabaccord_empty_elm_flag">('+wt_pklist_dc_params.labels.empty_item+')</span>' : '')
					+'</div>'
