...
var pklist_dc=(function( $ ) {
	...
	window.onbeforeunload = function(){
		var current_tab = $(".nav-tab-active").attr('href');
		if(current_tab.trim() === "#wf_woocommerce_packing_list-dc"){
			var dc_change = pklist_dc_source_code.get_source_html();
			if(dc_change != pklist_dc.template_content){
				return wt_pklist_dc_params.labels.prompt_leave_customizer;
			}
		}
	};
	var pklist_dc=
	{
		...
		col_delete_btn:'<div class="wt_pklist_dc_item_delete" title="'+wt_pklist_dc_params.labels.delete_block+'">X</div>',
		...
		template_loaded:0,
		open_first_panel:true, /* open first panel on editable item selected */
		open_editor_on_drag_from_assets:false, /* open block editor of current block dragged from assets */
		initial_tmr:null,
		page_wfte_name:'page_main',
		Set:function()
		{	
		...
		/**
		 * 	Hide empty elements in the template preview to avoid unncessary white spaces.	
		 */
		hide_empty_elements:function()
		{
			$('.wt_pklist_dc_visual_editor .wfte_from_address span').each(function(){
				if($(this).text().trim()==="")
				{
					$(this).addClass('wt_pklist_dc_empty_item');
				}
			});
		},
		...
		*/
		check_template_compatibility:function()
		{
			...
			if(editor_elm.find('.wfte_invoice-main').length>0 && editor_elm.find('.wfte_invoice-main').html().trim()==="") /* check for blank template */
			{
				incompatiblity=0; /* this is a valid blank template so no need to show the warning */
			}
			if(incompatiblity>0)
			{
		...
		*/
		reg_events:function()
		{
			...
			/**
			 *  Close choosed block on outer click
			 */
			$(document).on('click', '.wt_pklist_dc_visual_editor *', function(e){
				var elm=$(this);
				if(elm.hasClass('wfte_row'))
				{
					pklist_dc.remove_editable_selected();
				}
			});
			/**
			...
			*/
			...
			$(document).on('click', '.wt_pklist_dc_visual_editor .wt_pklist_dc_empty_column', function(e){				
				e.stopPropagation();
				$('.wt_pklist_dc_sidebar_tab_btn').removeClass('wt_pklist_active_tab');
				$('.wt_pklist_dc_sidebar_tab_btn[data-tab-target="wt-pklist-dc-sidebar-assets"]').addClass('wt_pklist_active_tab');
				$('.wt_pklist_dc_sidebar_tabcontent[data-tab-id="wt-pklist-dc-sidebar-page"]').hide();
				$('.wt_pklist_dc_sidebar_tabcontent[data-tab-id="wt-pklist-dc-sidebar-block"]').hide();
				$('.wt_pklist_dc_sidebar_tabcontent[data-tab-id="wt-pklist-dc-sidebar-assets"]').show();
				$('.wt_pklist_dc_empty_column').removeClass('wt_pklist_dc_dragover');
				$(this).addClass('wt_pklist_dc_dragover');
			});
			/**
...
function do_auto_complete_for_order_meta(){
	var order_meta_val_arr = JSON.parse(wt_pklist_dc_params.order_meta_autocomplete);
	jQuery(document).find("#dc_add_new_order_meta").autocomplete({
	   	source: order_meta_val_arr,
		autoFill: true,
		appendTo:".wt_pklist_dc_add_new_sub_item_popup_content",
		open: function(){
			jQuery(this).autocomplete('widget').css({'z-index':'1000000001','overflow-y': 'auto','overflow-x': 'hidden','max-height':'350px'});
       		return false;
		},
