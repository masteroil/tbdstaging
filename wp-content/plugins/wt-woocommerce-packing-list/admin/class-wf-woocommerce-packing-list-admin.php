...
class Wf_Woocommerce_Packing_List_Admin {
	...
	*/
	...
	public function enqueue_scripts() {
		...
		$params=array(
			...
			'tot_tax_amount_img' => WF_PKLIST_PLUGIN_URL.'admin/images/preview_tot_tax_amount_img.png',
			'tot_tax_rate_img' => WF_PKLIST_PLUGIN_URL.'admin/images/preview_tot_tax_rate_img.png',
			'tot_tax_amount_rate_img' => WF_PKLIST_PLUGIN_URL.'admin/images/preview_tot_tax_amount_rate_img.png',
			'ind_tax_amount_img' => WF_PKLIST_PLUGIN_URL.'admin/images/preview_ind_tax_amount_img.png',
			'ind_tax_rate_img' => WF_PKLIST_PLUGIN_URL.'admin/images/preview_ind_tax_rate_img.png',
			'ind_tax_amount_rate_img' => WF_PKLIST_PLUGIN_URL.'admin/images/preview_ind_tax_amount_rate_img.png',
			'ind_tax_seperate_img' => WF_PKLIST_PLUGIN_URL.'admin/images/preview_ind_tax_seperate_img.png',
			...
			'msgs'=>array(
				...
				'fitler_code_copied' => __("Code Copied","wf-woocommerce-packing-list"),
				'qr_placeholder_copied' => __("Placeholder Copied","wf-woocommerce-packing-list"),
			)
		);
	 ...
	 */
	public function add_email_print_actions($order, $sent_to_admin, $plain_text, $email=true )
	{
	...
	public function admin_menu()
	{
		...
		$menus[]=array(
			'submenu',
			WF_PKLIST_POST_TYPE,
		 	__('Addons','wf-woocommerce-packing-list'),
            __('Addons','wf-woocommerce-packing-list'),
            'manage_woocommerce',
    ...
    * Bug fix : Return order number instead of order_id
    */
    ...
    */
	public static function wf_shipping_formated_price($order,$template_type){
		$order_id=(WC()->version<'2.7.0' ? $order->id : $order->get_id());
		...
		$incl_tax_text='';
		$incl_tax_text=Wf_Woocommerce_Packing_List_CustomizerLib::get_tax_incl_text($template_type, $order);
	...
	public static function document_module_enable_disable(){
        ...
        if(!Wf_Woocommerce_Packing_List_Admin::check_write_access()) 
		{
			echo json_encode(array('status' => true, 'doc_set' => 0, 'message' => __('You are not allowed to do this action','wf-woocommerce-packing-list')));
			exit();
		}
	    ...
	    {	
	    	...
	    	$output['doc_set'] = 1;
	    	$output['message'] = __('Updated','wf-woocommerce-packing-list');	
	    }else{
	    	foreach($wt_pklist_common_modules as $k=>$v)
	        {
				$wt_pklist_common_modules[$k]=0;
	        ...
	        $output['doc_set'] = 2;
	        $output['message'] = __('No modules','wf-woocommerce-packing-list');
	    }
	    update_option('wt_pklist_common_modules',$wt_pklist_common_modules);
	...
	public function wt_pklist_delete_all_invoice_numbers_tool($tools){
		...
		$tool_description = sprintf('%1$s<br><strong class="red">%2$s</strong>',__( 'This will remove all invoice numbers created by WooCommerce PDF Invoices, Packing Slips, Delivery Notes & Shipping Labels by WebToffee.', 'wf-woocommerce-packing-list' ),__( 'Note:', 'wf-woocommerce-packing-list' ))." ".sprintf(__( 'Before using this tool, please make sure you followed the steps described in this article %1$s how to reset/delete existing invoice numbers%2$s.', 'wf-woocommerce-packing-list' ),'<a href="' . esc_url( $article_url ) . '">','</a>');
		$tools['wf_pklist_delete_all_invoice_number'] = array(
	        'name' => __('Delete all generated invoice numbers', 'wf-woocommerce-packing-list'),
	        'button' => __('Delete',  'wf-woocommerce-packing-list'), 
	...
	public static function wf_pklist_delete_all_invoice_numbers_func(){
		delete_post_meta_by_key( 'wf_invoice_number' );
	}

	public static function order_meta_dropdown_list(){
    	$order_meta_query = array();
    	if((isset($_GET['page'])) && ($_GET['page'] === "wf_woocommerce_packing_list_invoice")){
			global $wpdb;
    ...
    public function wt_pklist_action_scheduler_for_invoice_number(){
		...
		if(true === $data["invoice_enabled"] && $data["order_empty_invoice_count"] > 0){
			if((true === $data["auto_generate"]) && (100 < $data["order_empty_invoice_count"]) && (false === as_next_scheduled_action('wt_pklist_schedule_auto_generate_invoice_number', array(), $group))){
				as_enqueue_async_action('wt_pklist_schedule_auto_generate_invoice_number', array(), $group);
			}elseif(true === $data["auto_generate"] && (100 >= $data["order_empty_invoice_count"] && 0 < $data["order_empty_invoice_count"])){
				do_action('wt_pklist_auto_generate_invoice_number_module');
			}
		}else{
			if (true === as_next_scheduled_action('wt_pklist_schedule_auto_generate_invoice_number', array(), $group) && false === $data["invoice_enabled"]) {
	...
	public static function check_before_auto_generating_invoice_no(){
		...
		if(true === $result["auto_generate"] && true === $result["invoice_enabled"]){
			$result["order_empty_invoice_count"] = count(self::get_order_ids_for_invoice_number_generation($invoice_module_id));
		}
		return $result;
	}
	...
	public static function get_order_ids_for_invoice_number_generation($module_id){
		...
		if(!empty($generate_invoice_for)){
			$result_status_arr = "('" . implode ( "', '", esc_sql($generate_invoice_for) ) . "')";
			...
			$order_meta_query_arr = $wpdb->get_results(
					"SELECT p.`ID` FROM $wpdb->posts AS p WHERE p.`post_type` = 'shop_order'
					 AND p.`post_status` IN $result_status_arr
					 AND (NOT EXISTS(SELECT * FROM $wpdb->postmeta as m WHERE m.`meta_key` = 'wf_invoice_number'AND m.`post_id` = p.`ID`) OR 
					 		(EXISTS (SELECT * FROM $wpdb->postmeta as m WHERE m.`post_id` = p.`ID`
					 				AND m.`meta_key` = 'wf_invoice_number'
					 				AND (m.`meta_value` IS NULL OR m.`meta_value` = '')))) ORDER BY p.`ID` ASC"
		            );
	   		if("No" === $invoice_for_prev_install_order){
				...
				$local_timestamp = "'".get_date_from_gmt( $utc_timestamp_converted, 'Y-m-d h:i:s' )."'";
	   			...
	   			$order_meta_query_arr = $wpdb->get_results(
					"SELECT p.`ID` FROM $wpdb->posts AS p WHERE p.`post_type` = 'shop_order'
					 AND p.`post_status` IN $result_status_arr
					 AND p.`post_date` > $local_timestamp
					 AND (NOT EXISTS(SELECT * FROM $wpdb->postmeta as m WHERE m.`meta_key` = 'wf_invoice_number'AND m.`post_id` = p.`ID`) OR 
					 		(EXISTS (SELECT * FROM $wpdb->postmeta as m WHERE m.`post_id` = p.`ID`
					 				AND m.`meta_key` = 'wf_invoice_number'
					 				AND (m.`meta_value` IS NULL OR m.`meta_value` = '')))) ORDER BY p.`ID` ASC"
