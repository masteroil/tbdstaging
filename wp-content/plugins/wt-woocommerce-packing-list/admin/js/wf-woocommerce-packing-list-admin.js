(function( $ ) {
	...
	$(function() {
		...
		{
			var settings_btn=elm.parents('.wf_pklist_dashboard_box_footer_up').find('.doc_module_link');
			if(elm.is(':checked'))
			{
		...
		jQuery('[name="wt_pklist_custom_field_btn"]').on('click',function(){
	        ...
	        if(0 === empty_fields){
    			...
    			if (custom_meta_key !== "" && (jQuery.isNumeric(custom_meta_key) || custom_meta_key.match(/^\d+$/))) {
    				alert(wf_pklist_params.msgs.enter_mandatory_non_numeric_fields);
    				return false;
    			}
	        }
	        var elm=jQuery(this);
	...
	Set:function(){
		jQuery('.wt_document_module_enable').on('change',function(){
			...
			jQuery.ajax({
				...
				success:function(data)
				{
					if(data.status == true && data.doc_set == 1){
						var settings_btn = elm.parents('.wf_pklist_dashboard_box_footer_up').find('.doc_module_link');
						if(doc_module_set == 1){
							settings_btn.attr('href',settings_btn.attr('data-href')).css({'opacity':'1','cursor':'pointer'});
						}else{
...
var wt_pklist_individual_tax_column_option = {
	Set:function(){
		jQuery('[name="wt_pklist_individual_tax_column_display_option[]"]').change(function(){
			if(jQuery(this).is(':checked')){
				if((jQuery(this).val() == "amount") || (jQuery(this).val() == "rate")){
					jQuery('#wt_pklist_individual_tax_column_display_option_separate-column').prop("checked",false);
				}else if(jQuery(this).val() == "separate-column"){
					jQuery('#wt_pklist_individual_tax_column_display_option_amount').prop("checked",false);
					jQuery('#wt_pklist_individual_tax_column_display_option_rate').prop("checked",false);
				}
			}
		});
	}
...
var wt_pklist_tax_column_preview_images = {
	Set:function(){
		this.optionChange('loading');
		jQuery(document).find('input[name="wt_pklist_total_tax_column_display_option[]"]').on('change',function(){
			wt_pklist_tax_column_preview_images.optionChange('changing');
		});
	},

	optionChange:function(action){
		var module_base = jQuery('.wf_settings_base').val();

		if(module_base === "invoice" || module_base === "dispatchlabel" || module_base === "creditnote" || module_base === "proformainvoice"){
			var total_tax_col_arr = [];
			jQuery('input[name="wt_pklist_total_tax_column_display_option[]"]:checked').each(function(i){
	          	total_tax_col_arr[i] = jQuery(this).val();
	        });
			var prev_img_html = '<span class="wf_form_help wt_pklist_conditional_help_text" id="tot_tax_amount" style="display:none;"><img src="'+wf_pklist_params.tot_tax_amount_img+'" style="width:170px;padding:10px;background:#f0f0f1;"></span><span class="wf_form_help wt_pklist_conditional_help_text" id="tot_tax_rate" style="display:none;"><img src="'+wf_pklist_params.tot_tax_rate_img+'" style="width:170px;padding:10px;background:#f0f0f1;"></span><span class="wf_form_help wt_pklist_conditional_help_text" id="tot_tax_amount_rate" style="display:none;"><img src="'+wf_pklist_params.tot_tax_amount_rate_img+'" style="width:170px;padding:10px;background:#f0f0f1;"></span>';
	        if(action == "loading"){
	        	jQuery(document).find('input[name="wt_pklist_total_tax_column_display_option[]"]').parent().append(prev_img_html);
	        }
	        jQuery('#tot_tax_amount_rate').hide();
		    jQuery('#tot_tax_amount').hide();
		    jQuery('#tot_tax_rate').hide();
	        if((jQuery.inArray("amount", total_tax_col_arr) != -1) && (jQuery.inArray("rate", total_tax_col_arr) != -1)) {
			    jQuery('#tot_tax_amount_rate').show();
			}else if(jQuery.inArray("amount", total_tax_col_arr) != -1){
			    jQuery('#tot_tax_amount').show();
			}else if(jQuery.inArray("rate", total_tax_col_arr) != -1){
			    jQuery('#tot_tax_rate').show();
			}
		}
...
var wt_pklist_ind_tax_column_preview_images = {
	Set:function(){
		this.optionChange('loading');
		jQuery(document).find('input[name="wt_pklist_individual_tax_column_display_option[]"]').on('change',function(){
			wt_pklist_ind_tax_column_preview_images.optionChange('changing');
		});
	},
	...
	optionChange:function(action){
		...
		if(module_base == "invoice" || module_base == "dispatchlabel" || module_base == "creditnote" || module_base == "proformainvoice"){
			var total_tax_col_arr = [];
			jQuery('input[name="wt_pklist_individual_tax_column_display_option[]"]:checked').each(function(i){
	          	total_tax_col_arr[i] = jQuery(this).val();
	        });
			
			var prev_ind_img_html = '<span class="wf_form_help wt_pklist_conditional_help_text" id="ind_tax_amount" style="display:none;"><img src="'+wf_pklist_params.ind_tax_amount_img+'" style="width:250px;padding:10px;background:#f0f0f1;"></span><span class="wf_form_help wt_pklist_conditional_help_text" id="ind_tax_rate" style="display:none;"><img src="'+wf_pklist_params.ind_tax_rate_img+'" style="width:250px;padding:10px;background:#f0f0f1;"></span><span class="wf_form_help wt_pklist_conditional_help_text" id="ind_tax_amount_rate" style="display:none;"><img src="'+wf_pklist_params.ind_tax_amount_rate_img+'" style="width:250px;padding:10px;background:#f0f0f1;"></span><span class="wf_form_help wt_pklist_conditional_help_text" id="ind_tax_seperate_col" style="display:none;"><img src="'+wf_pklist_params.ind_tax_seperate_img+'" style="width:75%;padding:10px;background:#f0f0f1;"></span>';
	        if(action == "loading"){
	        	jQuery(document).find('input[name="wt_pklist_individual_tax_column_display_option[]"]').parent().append(prev_ind_img_html);
	        }

		    jQuery('#ind_tax_amount_rate').hide();
		    jQuery('#ind_tax_amount').hide();
		    jQuery('#ind_tax_rate').hide();
		    jQuery('#ind_tax_seperate_col').hide();
	        if((jQuery.inArray("amount", total_tax_col_arr) != -1) && (jQuery.inArray("rate", total_tax_col_arr) != -1)) {
			    jQuery('#ind_tax_amount_rate').show();
			}else if(jQuery.inArray("amount", total_tax_col_arr) != -1){
			    jQuery('#ind_tax_amount').show();
			}else if(jQuery.inArray("rate", total_tax_col_arr) != -1){
			    jQuery('#ind_tax_rate').show();
			}else if(jQuery.inArray("separate-column", total_tax_col_arr) != -1){
				jQuery('#ind_tax_seperate_col').show();
			}
		}
...
var wt_pklist_filter_actions = {
	Set:function(){
		...
		jQuery('.filter_code_copy').on('click',function(){
			...
			jQuery("body").append(temp);
			var elm = jQuery(this);
			console.log(elm.parent().parent().children('er').html());
...
jQuery(document).ready(function(){
	...
	wt_pklist_individual_tax_column_option.Set();
	wt_pklist_tax_column_preview_images.Set();
	wt_pklist_ind_tax_column_preview_images.Set();
