...
<div class="wf-tab-content" data-id="<?php echo $target_id;?>">
	...
	<table class="wf-form-table">
	    ...
	    self::generate_form_field(array(
			...
			array(
				'type'=>'radio',
				'label'=>__('Display price in the product table', 'wf-woocommerce-packing-list'),
				...
				'after_form_field'=>($is_tax_enabled ? '<a href="'.admin_url('admin.php?page=wc-settings&tab=tax').'" class="" target="_blank" style="text-align:center;">'.__('WooCommerce tax settings', 'wf-woocommerce-packing-list').'<span class="dashicons dashicons-external"></span></a>' : ""),
				'radio_fields'=>array(
				...
				'help_text_conditional'=>array(
	                array(
	                	'help_text'=>sprintf(__('All price columns displayed will be inclusive of tax.%s Learn how to add tax column in the documents. %s', 'wf-woocommerce-packing-list'),'<a href="'.esc_url($tax_doc_url).'" target="_blank">', '</a>').$tax_not_enabled_info.$incl_tax_img,
	                    'condition'=>array(
	                        array('field'=>'woocommerce_wf_generate_for_taxstatus', 'value'=>'in_tax')
	                    )
	                ),
	                array(
	                	'help_text'=>sprintf(__('All price columns displayed will be exclusive of tax.%s Learn how to add tax column in the documents. %s', 'wf-woocommerce-packing-list'),'<a href="'.esc_url($tax_doc_url).'" target="_blank">', '</a>').$tax_not_enabled_info.$excl_tax_img,
	                    'condition'=>array(
	    ...
	    */
		...
		$form_fields[]=array(
			...
			'help_text'=>sprintf(__('%sSingle package(per order)%s - All the items belonging to an order are packed together into a single package. Every order will have a respective package.','wf-woocommerce-packing-list'),'<b>','</b>').'<br />'.
				sprintf(__('%sBox packing(per order)%s - All the items belonging to an order are packed into the respective boxes as per the configuration. Every order may have one or more boxes based on the configuration.','wf-woocommerce-packing-list'),'<b>','</b>').'<br />'.
				sprintf(__('%sPack items individually%s - Every item from the order/s are packed individually. e.g if an order has 2 quantities of product A and 1 quantity of product B, there will be three packages consisting one item each from the order.','wf-woocommerce-packing-list'),'<b>','</b>'),
			'form_toggler'=>array(
				'type'=>'parent',
