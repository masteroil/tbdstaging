    ...
    public function invoice_number_action_scheduler_notice(){
        ...
        if((true === $auto_generate["invoice_enabled"]) && (true === $auto_generate["auto_generate"]) && (100 < $auto_generate["order_empty_invoice_count"]))
        {
            ...
            if (true === as_next_scheduled_action('wt_pklist_schedule_auto_generate_invoice_number', array(), $group)) {
                ...
                <h3 style="margin: 10px 0;">'.__("Invoice Generation In Progress","wf-woocommerce-packing-list").'</h3>
                <p>'.__("Invoice numbers are getting generated in the background. This process may take a little while, so please be patient.","wf-woocommerce-packing-list").'</p>
                <p><a class="button button-primary" href="'.esc_url($pending_actions_url).'">'.__("View progress","wf-woocommerce-packing-list").'</a></p>
                </div>';
            }
        }
    }
