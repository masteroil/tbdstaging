...
if((!class_exists('Wf_Woocommerce_Packing_List_Pay_Later_Payment')) && (class_exists('WC_Payment_Gateway')))
{
	class Wf_Woocommerce_Packing_List_Pay_Later_Payment extends WC_Payment_Gateway {
		...
		public function email_instructions( $order, $sent_to_admin, $plain_text = false ) {
			...
			$payment_method = (WC()->version< '2.7.0' ? get_post_meta( $order_id, '_payment_method', true ) : $order->get_payment_method());
		...
		public function process_payment( $order_id ) {
			...
			$order->update_status( 'on-hold', __( 'Awaiting offline payment', 'wf-woocommerce-packing-list' ) );
