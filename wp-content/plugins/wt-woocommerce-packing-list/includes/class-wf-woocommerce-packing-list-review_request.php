...
class Wf_Woocommerce_Packing_List_Review_Request
{
	...
	*/
	...
	private $review_url="https://www.webtoffee.com/testimonials/?product_id=90383#submit-your-review";
	private $plugin_prefix="wt_pklist_pro"; /* must be unique name */
	private $activation_hook="wt_pklist_pro_activate"; /* hook for activation, to store activated date */
	private $deactivation_hook="wt_pklist_pro_deactivate"; /* hook for deactivation, to delete activated date */
	...
	public function __construct()
	{
		...
		{
			$this->banner_message=sprintf(__("Hey, we at %s WebToffee %s would like to thank you for using our plugin. Less than a minute of your time will motivate us to keep doing what we do. We would really appreciate if you could take a moment to drop a quick review that motivate us to keep going.", 'wf-woocommerce-packing-list'), '<b>', '</b>');

			/* button texts */
			$this->later_btn_text=__("Remind me later", 'wf-woocommerce-packing-list');
			$this->never_btn_text=__("Not interested", 'wf-woocommerce-packing-list');
			$this->review_btn_text=__("Review now", 'wf-woocommerce-packing-list');

			if(1000 <= $this->doc_created_count){
				$this->webtoffee_logo_url = "";
				$round_count = floor($this->doc_created_count/100) * 100;
				$this->banner_message=sprintf( __( '%1$s You’ve just created %2$s using %3$s. That’s awesome! We’d love it if you take a moment to share what you think and help spread the word.', 'wf-woocommerce-packing-list' ), '<b>Knock, Knock!</b>', '<b style="color:#FF6636;font-size: 14px;">'.$round_count.'+ commercial documents </b>' ,'<b style="color:#FF6636;font-size: 14px;">Woocommerce PDF invoices, packing slips, delivery notes, and shipping labels</b>');
				$this->review_btn_text=__("Rate us now", 'wf-woocommerce-packing-list');
			}
	...
	*/
	public function set_vars()
	{
		...
		$this->doc_created_count = get_option('wt_created_document_count',true);		
	}
	...
	*/
	public function show_banner()
	{
		...
		if(1000 <= $this->doc_created_count){
			...
			?>
			<div class="<?php echo sanitize_text_field($this->banner_css_class); ?> notice-info notice is-dismissible">
	            <?php
	            if ($this->webtoffee_logo_url != "") {
	            ?>
	                <h3 style="margin: 10px 0;"><?php echo sanitize_text_field($this->plugin_title); ?></h3>
	            <?php
	            }
	            ?>
	            <p>
	                <?php echo sanitize_text_field($this->banner_message); ?>
	            </p>
	            <p>
	                <a class="button button-secondary" style="color:#333; border-color:#ccc; background:#efefef;" data-type="later"><?php echo sanitize_text_field($this->later_btn_text); ?></a>
	                <a class="button button-primary" data-type="review"><?php echo sanitize_text_field($this->review_btn_text); ?></a>
	            </p>
		...
		if(isset($_POST['wt_review_action_type']))
		{
			...
			if(in_array($action_type, $this->allowed_action_type_arr))
			{
				if($action_type==='never' || $action_type==='closed')
				{
					...
					$this->reset_start_date();
					if(1000 <= $this->doc_created_count){
						$new_banner_state=33; // To wait for the value based review seeking banner
					}
				}
	...
	public function add_banner_scripts()
	{
		...
		<script type="text/javascript">
		    (function($){
		        ...
		        $(document).on('click', '.<?php echo sanitize_text_field($this->banner_css_class);?> a.button', function(e)
		        {
		            e.preventDefault();
		            var elm=$(this);
		            ...
		            elm.parents('.<?php echo sanitize_text_field($this->banner_css_class);?>').hide();
		        ...
		        }).on('click', '.<?php echo sanitize_text_field($this->banner_css_class);?> .notice-dismiss', function(e)
		        {
	                e.preventDefault();
