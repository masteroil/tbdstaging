...
function process_skip_next_date( $user_id, $subscription, $nonce ) {
	...
	if (
		...
		$next_payment_date = apply_filters( 'sptb_next_payment_date_after_skip', $next_payment_date, $subscription );
		...
		// save skip meta to hide button in future
		update_post_meta( $subscription->get_id(), 'wcs_toolbox_skipped', 1 );
 ...
 * accessed from children classes too, but not outside, and handles all validations. Assuming nonce generated
...
function process_ship_now_adjust_date( $user_id, $subscription, $nonce ) {
	...
	if (
		...
		do_action( 'jgtb_after_ship_now_adjust_date', $subscription, $user_id, $old_next_date, $next_payment_date );
...
function process_change_shipping_address( $user_id, $subscription, $nonce ) {
	if (
		...
		$new_shipping_address = Utilities\sieve_address_details( $_REQUEST, 'shipping' ); // phpcs:ignore CSRF okay.
		...
		if ( ! empty( $shipping_diff ) ) {
			...
			foreach ( $shipping_diff as $key => $value ) {
				$shipping_address_string .= PHP_EOL . '[' . $key . ']: ' . $value;
			}
			$subscription->set_address( $shipping_diff, 'shipping' );
			$subscription->add_order_note( sprintf( 'Customer chose to update the shipping details with: %s', $shipping_address_string ), false, true );
...
function process_change_billing_address( $user_id, $subscription, $nonce ) {
	if (
		...
		$new_billing_address = Utilities\sieve_address_details( $_REQUEST, 'billing' ); // phpcs:ignore CSRF ok
		...
		if ( ! empty( $billing_diff ) ) {
			...
			foreach ( $billing_diff as $key => $value ) {
				$billing_address_string .= PHP_EOL . '[' . $key . ']: ' . $value;
			}

			$subscription->set_address( $billing_diff, 'billing' );
			$subscription->add_order_note( sprintf( 'Customer chose to update the billing details with: %s', $billing_address_string ), false, true );
	...
	if (
		...
		if ( count( $subscription_items ) > 0 ) {
			foreach ( $subscription_items as $item_id => $item ) {
				$new_qty = ( isset( $_REQUEST[ 'new_quantity_' . $item_id ] ) ) ? $_REQUEST[ 'new_quantity_' . $item_id ] : null; // phpcs:ignore CSRF ok
				$old_qty = $item->get_quantity();
				if ( null !== $new_qty && $new_qty !== $old_qty ) { // phpcs:ignore Standard.Category.SniffName.ErrorCode
					$qty_changed         = true;
					$product_id          = wcs_get_canonical_product_id( $item );
					$product             = wc_get_product( absint( $product_id ) );
					$item_name           = wcs_get_line_item_name( $item );
					$changed[ $item_id ] = array(
		...
		do_action( 'jgtb_after_not_changed_product_quantities', $subscription, $changed );
...
/**
 * Change frequency. Hooked into priority 30, because this needs to happen after the change frequency processing is
 * done. Otherwise this would first change the next payment date of the subscription making the nonce that relied on
 * the previous next payment date invalid for the change next payment date function which would throw a notice then.
 *
 * I've gotten around that two ways:
 * 1. first change the next payment date, as the nonce for this function does not depend on the next payment date, so
 * both will successfully run, and
 * 2. only run the change next payment date function if the user changed the next payment date.
 *
 * If the user changed the frequency, the next payment date will be recalculated from today anyways though, so in that
 * sense any change that they've done to the next payment date will be irrelevant and they'd have to do it again.
 *
 * @param integer          $user_id      ID of the current user
 * @param \WC_Subscription $subscription Subscription we're changing the frequency for
 * @param string           $nonce        a security nonce
...
function process_change_frequency( $user_id, $subscription, $nonce, $new_interval, $new_period ) {
	if (
		...
		$next_ship_date = $subscription->get_date( 'next_payment' );

		$old_interval = $subscription->get_billing_interval();
		$old_period   = $subscription->get_billing_period();

		if ( $new_interval != $old_interval ) { // phpcs:ignore loose comparison ok.
			$subscription->set_billing_interval( $new_interval );
		}

		if ( $new_period != $old_period ) { // phpcs:ignore loose comparison ok.
			$subscription->set_billing_period( $new_period );
		}

		/**
		 * Filter whether to change the next_payment date of the subscription when frequency is changed.
		 *
		 * @since 1.4.22
		 *
		 * @param bool $update_next_date default true
		 * @param int $user_id ID of the user making the change
		 * @param \WC_Subscription $subscription Subscription the frequency is changing
		 */
		if ( true === (bool) apply_filters( 'sptb_frequency_change_next_payment', true, $user_id, $subscription ) ) {
			$next_ship_date = gmdate( 'Y-m-d H:i:s', strtotime( '+' . $new_interval . ' ' . $new_period ) );
...
function process_change_next_shipping_date( $user_id, $subscription, $nonce, $new_ship_date ) {
	if (
		...
		$new_ship_date = DateTime::createFromFormat( 'Y-m-d', $_REQUEST['new_ship_date'], $tz ); // phpcs:ignore CSRF ok
		...
		if ( $adjust_end_date ) {
			...
			/**
			 * Filter for end date when it is changed.
			 *
			 ...
			 * @since 1.4.24
			 */
			$end_date = apply_filters( 'sptb_new_end_date', $end_date, $subscription, $user_id );

			$dates['end'] = $end_date ? $end_date->format( 'Y-m-d H:i:s' ) : $end_date;
