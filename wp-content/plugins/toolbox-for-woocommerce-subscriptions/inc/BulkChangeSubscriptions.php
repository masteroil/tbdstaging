 ...
 * @param $all_subscriptions_hash
 * @param $remote_hash
 * @return false|mixed|string           array of subscription ids (or empty), or false, if there was a problem
 */
...
/**
 * Loops through the checkboxes, and handles individual reschedules. The nonce for individual subscriptions are then
...
function process_bulk_ship_now_keep( $user_id, $subscription_ids ) {
	...
	foreach ( $subscription_ids as $id ) {
		...
		$nonce        = isset( $_REQUEST[ '_completed_' . $id ] ) ? $_REQUEST[ '_completed_' . $id ] : ''; // phpcs:ignore CSRF ok. Will be double checked in process_ship_now_keep_date
...
function process_bulk_ship_now_reschedules( $user_id, $subscription_ids ) {
	...
	foreach ( $subscription_ids as $id ) {
		$subscription = wcs_get_subscription( $id );
		$nonce        = isset( $_REQUEST[ '_completed_adjust_' . $id ] ) ? $_REQUEST[ '_completed_adjust_' . $id ] : ''; // phpcs:ignore CSRF ok. Will be checked in the process_ship_now_adjust_date
...
/**
 * Loops through checkboxes, and handles individual reschedules. The nonce for individual subscriptions are then
 * grabbed from the REQUEST array based on the current integer.
 *
 * @param integer $user_id         ID of the current user
...
function process_bulk_change_quantities( $user_id, $subscriptions ) {
	...
	foreach ( $subscriptions as $subscription_id ) {
		...
		$nonce        = isset( $_REQUEST[ 'wcs_edit_details_of_' . $subscription_id ] ) ? $_REQUEST[ 'wcs_edit_details_of_' . $subscription_id ] : ''; // phpcs:ignore CSRF ok. Will be checked in process_change_product_quantities
 ...
 *
 * @param  array        $status     an array of ( 'succeeded' => array(), 'failed' => array() ) with subscription IDs as the values
 * @param  integer      $user_id    ID of the user requesting the change
 */
function display_ship_now_notice( $status, $user_id ) {
	$logger = wc_get_logger();
	$logger->debug( sprintf( 'Bulk ship now request from user id %s for the following subscriptions: %s', $user_id, get_status_string( $status ) ), array( 'source' => 'jgtb_bulk_actions' ) );
...
function display_quantity_notice( $status, $user_id ) {
	...
	$logger = wc_get_logger();
	$logger->debug( sprintf( 'Bulk quantity update request from user id %s for the following subscriptions:%s', $user_id, get_status_string( $status ) ), array( 'source' => 'jgtb_bulk_actions' ) );
...
/**
 * Convert status array to a string appropriate for logging.
 *
 * @param array $status
 * @return string
 */
function get_status_string( $status ) {
	$status_string = 'Succeeded:' . PHP_EOL;
	$succeeded     = $status['succeeded'] ?? array();
	$failed        = $status['failed'] ?? array();

	if ( count( $succeeded ) ) {
		foreach ( $succeeded as $key => $value ) {
			$status_string .= '[' . $key . '] : ' . $value . PHP_EOL;
		}
	} else {
		$status_string .= 'None.' . PHP_EOL;
	}
	$status_string .= 'Failed:' . PHP_EOL;
	if ( count( $failed ) ) {
