...
if ( ! class_exists( '\Javorszky\Toolbox\Emails\AdminNextPaymentChanged', false ) ) :

	/**
	 * Class for sending email to administrator after a customer changes the next payment date of a subscription.
	 */
	class AdminNextPaymentChanged extends \WC_Email {

		/**
		 * Constructor.
		 */
		public function __construct() {
			$this->id             = 'admin_next_payment_changed';
			$this->title          = __( 'Subscription Date Changed (Admin)', 'jg-toolbox' );
			$this->description    = __( 'This email is sent to the administrator when a customer changes the next renewal date of a subscription.', 'jg-toolbox' );
		...
		public function get_default_heading() {
			return __( 'Subscription Payment Date Changed: #{order_number}', 'jg-toolbox' );
		}
		...
		public function get_default_additional_content() {
			return __( 'Customer has changed payment date on Subscription #{order_number}. Next Date: {next_date}', 'jg-toolbox' );
		}
...
return new AdminNextPaymentChanged();
