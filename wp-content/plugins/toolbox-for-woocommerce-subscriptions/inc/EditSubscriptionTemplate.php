...
/**
 * Subscriptions adds a nonce / http referrer after the edit address forms for their own purposes.
 * Because we don't need that functionality, let's remove them.
 */
 ...
 * @param  string          $load_address the type of address to load. Defaults to billing.
 * @return array                         address fields with value, default, type, label.
 */
function get_address( $subscription, $load_address = 'billing' ) {
...
/**
 * Changes the date of the subscription if it's been requested and make sure all data is in order.
 * @throws Exception
 */
function handle_edit_details() {
	if ( isset( $_REQUEST['jgtb_edit_subscription_details'] ) && isset( $_REQUEST[ 'jgtb_edit_details_of_' . absint( $_REQUEST['jgtb_edit_subscription_details'] ) ] ) ) {
		...
		if ( isset( $_POST['sp_new_variation'] ) && $_POST['sp_new_variation'] ) {

			$items_changed = false;

			/** @var \WC_Order_Item_Product $item */
			foreach ( $subscription->get_items() as $item ) {
				if ( ! isset( $_POST['sp_new_variation'][ $item->get_id() ] ) ) {
					continue;
				}

				$new_variation_id = absint( $_POST['sp_new_variation'][ $item->get_id() ] );
				// It's the same. Move on.
				if ( absint( $item->get_variation_id() ) === $new_variation_id ) {
					continue;
				}
				...
				$subscription->add_product( $variation, $item->get_quantity(), array( 'variation' => $attributes ) );
			...
			if ( $items_changed ) {
				...
				wc_add_notice( esc_html__( 'Product Variation updated.', 'jg-toolbox' ) );
...
/**
 * Get the view subscription template. A post WC v2.6 compatible version of @see
 * WCS_Template_Loader::add_view_subscription_template()
 *
 * @since 2.0.17
 */
