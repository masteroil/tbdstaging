...
function add_ship_reschedule_action( $actions, $subscription ) {
	...
	if ( 0 !== $next_timestamp && 'active' === $subscription->get_status() ) {
		...
		$action_text = Utilities\get_button_text( 'ship_reschedule_button_text', 'Ship now and recalculate from today' );

		if ( \Javorszky\Toolbox\Utilities\is_ship_reschedule_from_next_payment_available() ) {
			$action_text = Utilities\get_button_text( 'ship_reschedule_from_next_payment_button_text', 'Ship now and reschedule from next payment' );
		}

		$new_actions = array(
			'ship_now_recalculate' => array(
				'url'  => get_ship_now_adjust_date_link( $subscription ),
				'name' => Utilities\replace_key_dates( $action_text, $subscription ),
			),
		);
